...
>   , specColors   :: V.Vector (V3 GLfloat)
...
> cuboid l' h' d' =
>   MeshSpec positions colors normals uvs indices where
...
>            -> V.Vector (V2 GLfloat)
>            -> V.Vector GLfloat
> interleave positions colors normals uvs =
>   V.foldr (V.++) V.empty $
>     V.zipWith4 combine positions colors normals uvs
...
>       <*> glGetAttribLocation  `forString` "color"
...
>       <*> glGetUniformLocation `forString` "diffuseColor"
>       <*> glGetUniformLocation `forString` "ambientColor"
>       <*> glGetUniformLocation `forString` "specularColor"
...
>   glEnableVertexAttribArray (colors glShader)
...
>   glVertexAttribPointer (colors glShader)
...
> draw resize state projectionMatrix = do
>   let 
>      viewMat  =       lookAt(cameraPosition state) (V3 0 0 0) (V3 0 1 0)
...
>   printErrors()
>   
>    glUseProgram ∘ shaderProgram $ shader resize
>   glBindVertexArray ∘ meshVAO $ mesh resize
>   glActiveTexture GL_TEXTURE0
>   glBindTexture GL_TEXTURE_2D $ texture resize
>   with pvmMat $
>     glUniformMatrix4fv (pvmMatrix $ shader resize)
>                        1 GL_TRUE ∘ castPtr
>   with viewModelMat $ 
>     glUniformMatrix4fv (viewModelMatrix $ shader resize)
>                        1 GL_TRUE ∘ castPtr 
>   with normalMat $
>     glUniformMatrix3fv (normalMatrix $ shader resize)
>                        1 GL_TRUE ∘ castPtr
>                        
>   glUniform4f (diffuseColor $ shader resize)   0.6 0.6 0.6 1
>   glUniform4f (ambientColor $ shader resize)   0.1 0.1 0.1 1
>   glUniform4f (specularColor $ shader resize)  0.7 0.7 0.7 1
>   glUniform1f (shininess $ shader resize)      0.4
>   glUniform3f (lightDirection $ shader resize) 0 0 1
>   glUniform1i (diffuseMap $ shader resize)     0
>
>   glDrawElements GL_TRIANGLES
>                  (meshIndexCount $ mesh resize)
>                  GL_UNSIGNED_INT
>                  (wordPtrToPtr 0)
>                  
